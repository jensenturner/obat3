$("#rs_route_id").change(function() {
  console.log('changed');
  $.getJSON("/i", {"agency_id": $("#rs_agency_id").val(), "route_id": $(this).val()}, function(data) {
    console.log('got');
    let stop_select = $("#rs_stop_id");
...
$("#rs_stop_id").change(function() {
  console.log('changed');
  $.getJSON("/i", {"agency_id": $("#rs_agency_id").val(), "route_id": $("#rs_route_id").val(), "stop_id": $(this).val()}, function(data) {
    console.log('got');
    let trip_select = $("#rs_trip_id");
...
$("#af_agency_id").change(function() {
  console.log('changed');
  $.getJSON("/i", {"agency_id": $(this).val(), "get_fleets": true}, function(data) {
    console.log('got');
    let fleet_select = $("#af_fleet_id");
    fleet_select.html('<option value="0" selected disabled>Select...</option>');
    $.each(data, function() {
      console.log(this.id);
      fleet_select.get(0).options.add(new Option(`${this.name} (${this.engine}/${this.transmission})`, this.id));
...
$("#rt_route_id").change(function() {
  console.log('changed');
  $.getJSON("/i", {"agency_id": $("#rt_agency_id").val(), "route_id": $(this).val()}, function(data) {
    console.log('got');
    let stop_select = $("#rt_stop_id");
    stop_select.html('<option value="0" selected disabled>Select...</option>');
    $.each(data, function() {
      stop_select.get(0).options.add(new Option(`${this.agency_code} ${this.description} (${this.direction} bound)`, this.id));
    });
...
$("#rt_stop_id").change(function() {
  console.log('changed');
  $.getJSON("/i", {"agency_id": $("#rt_agency_id").val(), "route_id": $("#rt_route_id").val(), "stop_id": $(this).val()}, function(data) {
    console.log('got');
    let trip_select = $("#rt_trip_id");
    trip_select.html('<option value="0" selected disabled>Select...</option>');
    $.each(data, function() {
      trip_select.get(0).options.add(new Option(`${moment(this.scheduled_time, 'HH:mm:ss:SSSS').format('h:mm A')}`, this.id));
    });
...
$("#rt_agency_id").change(function() {
  console.log('changed');
  $.getJSON("/i", {"agency_id": $(this).val(), "get_routes": true}, function(data) {
    console.log('got');
    let route_select = $("#rt_route_id");
    route_select.html('<option value="0" selected disabled>Select...</option>');
    $.each(data, function() {
      route_select.get(0).options.add(new Option(`${this.number} ${this.description}`, this.id));
    });
